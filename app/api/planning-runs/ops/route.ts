...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [runGroups, laneGroups, usageGroups, ownerRows, claimableRows, activeLeaseRows, expiredLeaseRows, archivedRunRows, oldestClaimableRows] = await Promise.all([
      ...
      db.select({ status: researchLaneJobs.status, count: sql<number>`count(*)` }).from(researchLaneJobs)
        .innerJoin(planningRuns, eq(researchLaneJobs.runId, planningRuns.id))
        .where(isNull(planningRuns.archivedAt)).groupBy(researchLaneJobs.status),
      ...
      db.select({ count: sql<number>`count(*)` }).from(planningRuns).where(and(
        isNotNull(planningRuns.leaseExpiresAt), lte(planningRuns.leaseExpiresAt, generatedAt),
        ne(planningRuns.currentStage, "published"), inArray(planningRuns.status, claimableStatuses),
      )),
