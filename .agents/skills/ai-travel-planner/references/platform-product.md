...
- `ResearchLaneJob`：按用户选择动态创建的主题研究线持久化状态；历史、文化、风景、美食只是默认项，建筑、博物馆、艺术、在地生活、亲子、夜游、购物、户外、摄影、康养、信仰、影视动漫和目录外自定义兴趣均可成为独立研究线。每条线记录尝试次数、证据数、Markdown、错误和完成时间。
...
- 路线：每天 N 个地点产生 N−1 个相邻请求。用户改变顺序、插入或移除地点时，服务端先删除该日失效路段、持久化新 Assignment 顺序，再把运行放回路线队列；不得只在浏览器内临时改图。
