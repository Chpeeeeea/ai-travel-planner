...
3. **Research Worker 接管**：受信执行器读取 `brief` 任务，按用户从 16 项目录中选择的主题动态拆分研究线；历史、文化、风景、美食只是默认组合，建筑、博物馆、艺术、在地生活、亲子、夜游、购物、户外、摄影、康养、信仰与影视动漫等都可独立检索，目录外要求进入自定义研究线。官方页面负责事实，小红书负责体验与现场摩擦，OSM 补名称和地理线索。结果通过 `/api/planning-runs/research` 分批写入，每条证据带来源，仍不包含高德 ID。
...
3. 增加按用户的任务数、POI 调用和路线调用配额。
4. 增加可分享但可撤销的只读旅行链接，并与私人编辑链接分离。
