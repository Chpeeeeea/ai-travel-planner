...
13. **管理任务**：未完成任务可以停止，系统释放 Worker 租约并拒绝后续阶段写入；已停止、失败或已发布任务可以归档并恢复，历史证据和真实调用记录不会被删除。
...
- 取消任务会阻止新批次；如果取消恰好发生在一个已经发送的高德小批次中，该批次最多完成 5 个调用，系统照实记账并保持任务为已取消。
...
- `PATCH /api/trips`：停止自己的未完成任务，或归档、恢复终态任务；取消会释放 Worker 租约。
